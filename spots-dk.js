/**
 * FishCast — Danish Fishing Spots Database
 * Source: fishingindenmark.info (Fishing in Denmark / Lystfisker Danmark)
 * Partners: Ministry of Food, Agriculture and Fisheries · DTU Aqua · VisitDenmark
 * Attribution: Data courtesy of fishingindenmark.info
 * Last compiled: June 2026 — verify current conditions at fishingindenmark.info
 */
...
/** Find spots within `radiusKm` of (lat, lon), sorted by distance */
DK_SPOTS.findNearby = function(lat, lon, radiusKm = 50) {
  return this
    .filter(s => typeof s === 'object' && s.lat)
    .map(s => ({
      ...s,
      distKm: Math.round(haversineSpots(lat, lon, s.lat, s.lon))
    }))
    .filter(s => s.distKm <= radiusKm)
    .sort((a, b) => a.distKm - b.distKm);
};

/** Which spots have the target species active in month m (1-12)? */
DK_SPOTS.withSpeciesInMonth = function(speciesNameEn, month) {
  return this
    .filter(s => typeof s === 'object' && s.species)
    .filter(s => s.species.some(sp =>
      sp.nameEn.toLowerCase() === speciesNameEn.toLowerCase() &&
      sp.months.includes(month)
    ));
};

/** Get species active at a spot in a given month */
DK_SPOTS.activeSpecies = function(slug, month) {
  const spot = this.find(s => s.slug === slug);
  if (!spot) return [];
  return spot.species.filter(sp => sp.months.includes(month));
};

function haversineSpots(lat1, lon1, lat2, lon2) {
